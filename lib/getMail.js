...
async function main(email, fromEmails, options) {
	...
	let res = await mail.getLastMailFromBox(fromEmails, getDataFromMail);

	if (res === null || lastDate >= res.attributes.date) {

		await mail.getBoxes();
		res = await mail.getLastMailFromBox(fromEmails, getDataFromMail, mail.junk);

		if ((res === null || lastDate >= res.attributes.date) && options.checkAllBoxes) {
			res = await mail.getLastMailFrom(fromEmails, getDataFromMail, lastDate);
		}
	...
	return main(email, fromEmails, options);
